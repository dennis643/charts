...
2. In Slack, DM candobro (channel D0BBBH668TE). Ask for TODAY's full report row as a
   SINGLE one-line JSON object with EXACTLY these keys (integers only; milestone = one
   short sentence):
...
3. Retry until candobro answers: after sending, wait ~60s and read the DM. If it replies
   with the "I'm being rate-limited" message (or is silent), wait ~2–3 min and resend —
   keep looping for up to ~2h. Only proceed once you have valid JSON.
